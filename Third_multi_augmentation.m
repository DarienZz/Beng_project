...
noise_level_db = 5; % Define the noise level before using it
num_iterations = 5;
...
function add_noise_and_save(subjects, base_dir, noise_level_db, num_iterations)
    ...
    for s = subjects
        % Define the directory where you want to save the .mat file for the current subject
        ...
        if ~exist(data_file, 'file')
            warning('File does not exist: %s', data_file);
